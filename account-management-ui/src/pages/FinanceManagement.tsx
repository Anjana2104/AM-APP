...
import {
  ...
  Segmented, InputNumber, Spin, Popconfirm, Modal, Form, Tag, Switch, Dropdown, Alert,
} from 'antd';
    ...
    try {
      ...
      uploaded.forEach(u => {
        ...
        if (existingMap.has(key)) {
          ...
          existingMap.set(key, { ...ex, company: u.company, space: u.space, owner: u.owner, revenue: mergedRevenue });
        } else {
      ...
      // Save to server outside any useState updater (avoids React Strict Mode double-invoke)
      const apiProjects = merged.map(r => ({
        id: r.id,
        project: r.project, company: r.company || '', code: r.code,
      ...
      if (saveResult.ok) {
        ...
        // Reload from server to get DB-assigned IDs for all rows (critical for future renames/updates)
        await reloadFromServer();
        ...
        <Space direction="vertical" style={{ width: '100%' }} size={12}>
          ...
          <div style={{ border: '1px solid #f0f0f0', borderRadius: 8, padding: '12px 16px' }}>
            ...
            <div style={{ fontSize: '11px', color: '#8c8c8c', marginBottom: 8 }}>
              Add an empty project row. Revenue columns will be pre-filled with 0 for all existing months.
            </div>
          ...
          <div style={{ border: '1px solid #fff1f0', borderRadius: 8, padding: '12px 16px', background: '#fff1f0' }}>
            ...
            <div style={{ fontSize: '11px', color: '#8c8c8c', marginBottom: 8 }}>
              Permanently removes all projects and revenue from the database. This cannot be undone.
            </div>
            <Popconfirm
