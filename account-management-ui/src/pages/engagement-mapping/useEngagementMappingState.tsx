...
import { Button, Space, Tag, Upload, Tooltip, message, Typography, Input, Select, AutoComplete, Collapse, Empty, Slider, Table, Statistic, Progress, Checkbox, Modal, Form, DatePicker } from 'antd';
...
import type { ResourceUtilizationProps, UnifiedFilterState } from './types';
import { DEFAULT_UNIFIED_FILTERS } from './types';

const { Text } = Typography;

export function useEngagementMappingState({ resources: propResources = [], onUpdateResources, onNavigate, onNavigateToRequest, onNavigateToInsights }: ResourceUtilizationProps) {
  ...
  }, [dispatch, onUpdateResources]);

  const buildAllocationForm = (resources: ResourceRow[]) => {
    if (!resources.length) {
      return { engagementName: '', beelineId: '', notes: '', engagementStartDate: '', engagementEndDate: '' };
    }
    const first = resources[0];
    const getCommonValue = (picker: (resource: ResourceRow) => string) => {
      const firstValue = picker(first);
      return resources.every((resource) => picker(resource) === firstValue) ? firstValue : '';
    };
    return {
      engagementName: getCommonValue((resource) => String(resource.engagement || '')),
      beelineId: getCommonValue((resource) => String(resource.beelineId || '')),
      notes: '',
      engagementStartDate: getCommonValue((resource) => String(resource.engagementStartDate || '')),
      engagementEndDate: getCommonValue((resource) => String(resource.engagementEndDate || '')),
    };
  };
  ...
  useEffect(() => {
    ...
    resourceApi.getResources().then(({ resources: apiRows, fromServer: online }) => {
      if (online && apiRows.length > 0) {
        const mapped: ResourceRow[] = apiRows.map((r: any, i: number) => mapResourceApiRowToResourceRow(r, i));
      ...
      render: (_: any, record: ResourceRow) => {
        ...
        return (
          <Space size={4} wrap>
            ...
            {status === 'Joined' && canEdit && (
              <Tooltip title="Edit engagement details" overlayInnerStyle={{ fontSize: '11px' }}>
                <Button type="text" size="small" icon={<EditOutlined style={{ color: '#1890ff' }} />} onClick={() => openEditModal(record)} />
              </Tooltip>
            )}
          </Space>
        );
  ...
  const downloadDeploymentPoolTemplate = () => {
    writeAoaSheetFile(
      ...
      'Deployment Pool Update',
      'Deployment_Pool_Update_Template.xlsx',
      { columnWidths: [14, 32, 20] },
    );
  ...
  const handleDeploymentPoolUpload = (file: File) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        publishResources(updatedResources);
  ...
  const openAllocationDrawer = async (resources: ResourceRow[], targetStatus = 'Shortlisted') => {
    ...
    setAllocationForm(buildAllocationForm(resources));
  ...
  const handleAllocateResource = (resource: ResourceRow) => {
    openAllocationDrawer([resource], resource.allocationStatus && resource.allocationStatus !== 'Available' ? resource.allocationStatus : 'Shortlisted');
  };
  ...
  const handleExportPipeline = () => {
    ...
    handleExportListTable(pipelineData, `Pipeline_Export_${getCurrentDateStamp()}.xlsx`);
  };
  ...
  const downloadEngagementTemplate = () => {
    writeAoaSheetFile(XLSX, [['RA ID', 'Current Engagement']], 'Engagement Template', 'Engagement_Update_Template.xlsx');
  };
...
export type EngagementMappingState = ReturnType<typeof useEngagementMappingState>;
