...
import { GenerateFYModal } from './GenerateFYModal';
import type { ExcelRow, InvRow } from './invoiceTypes';
import {
  deriveCode,
  downloadTemplate,
  formatCommentDate,
...
export function InvoiceListTable({ onDataChange, onMonthsChange }: InvoiceListTableProps) {
  ...
  const sharedInvoiceMonths = useAppSelector((state) => state.financeData.invoiceMonths);
  const sharedInvoiceLoaded = useAppSelector((state) => state.financeData.invoiceLoaded);
  const sharedInvoiceFromServer = useAppSelector((state) => state.financeData.invoiceFromServer);
  ...
  const companyOptions = getLinkedConfigLabelOptions(configs, 'invoice_company_field', 'InvoiceListTable');
  ...
  const financeProjects = sharedFinanceProjects as FinanceProject[];
  ...
  function mapInvoiceProjectsToRows(projects: invoiceApi.InvoiceProject[], months: string[]): InvRow[] {
  ...
  useEffect(() => {
    if (!sharedInvoiceLoaded || dirty) return;
    const mapped = mapInvoiceProjectsToRows(sharedInvoiceProjects, sharedInvoiceMonths);
    setRows(mapped);
    setMonthHeaders(sharedInvoiceMonths);
    onDataChange?.(mapped);
    onMonthsChange?.(sharedInvoiceMonths);
    setFromServer(sharedInvoiceFromServer);
    setPendingDeletedProjectIds([]);
  }, [dirty, onDataChange, onMonthsChange, sharedInvoiceFromServer, sharedInvoiceLoaded, sharedInvoiceMonths, sharedInvoiceProjects]);

  useEffect(() => {
    if (sharedInvoiceLoaded) {
      setLoading(false);
      return;
    }
    setLoading(true);
    invoiceApi.getInvoiceProjects().then(({ projects, months, fromServer: online }) => {
      dispatch(setInvoiceData({ projects, months, fromServer: online }));
      if (online && projects.length) {
        const mapped = mapInvoiceProjectsToRows(projects, months);
        setRows(mapped);
  ...
  }, [dispatch, onDataChange, onMonthsChange, sharedInvoiceLoaded]);

  useEffect(() => {
    if (sharedFinanceLoaded) return;
    financeApi.getProjects().then(({ projects, months, fromServer }) => {
      dispatch(setFinanceData({ projects, months, fromServer }));
    }).catch(() => { /* ignore — cross-check is best-effort */ });
  }, [dispatch, sharedFinanceLoaded]);
  ...
  const handleExport = () => {
    ...
    XLSXStyle.writeFile(wb, `Invoice_Export_${getCurrentDateStamp()}.xlsx`);
  ...
  return (
    <div>
      ...
      <GenerateFYModal
        ...
        canEdit={canEdit}
        canDelete={canDelete}
        generateFY={generateFY}
        candidateFYs={candidateFYs}
        hasRows={rows.length > 0}
        onGenerateFYChange={setGenerateFY}
        onGenerate={() => { if (generateFY) { handleGenerateFY(); setMoreActionsOpen(false); } }}
        onAddProject={() => { setMoreActionsOpen(false); setAddModalOpen(true); }}
        onDeleteAll={() => { void handleClearAll(); setMoreActionsOpen(false); }}
