...
export default function ResourceInsights({ resources: propResources = [], onNavigate, onNavigateWithFilter, onNavigateToRequest, onNavigateToProcess }: ResourceInsightsProps) {
  ...
  const handleDeleteComment = useCallback(async (commentId: number) => {
    ...
    await resourceApi.deleteResourceComment(selectedResourceId, commentId);
      ...
      children: (
        <div style={{ paddingTop: 4 }}>
          {(commentsBySection.general || []).length === 0 ? (
            ...
            (commentsBySection.general || []).map(c => <CommentMiniCard key={c.id} comment={c} currentUser={defaultAuthor} onDelete={handleDeleteComment} />)
          )}
        </div>
      ),
    },
                    ...
                    <>
                      {searchResults.groups.map(({ section, entries: sEntries, comments: sComments }) => (
                        <div key={section} style={{ marginBottom: 18 }}>
                          ...
                          {sComments.map(c => <CommentMiniCard key={`c-${c.id}`} comment={c} currentUser={defaultAuthor} onDelete={handleDeleteComment} />)}
                        </div>
                      ))}
                      {searchResults.generalComments.length > 0 && (
                        <div style={{ marginBottom: 18 }}>
                          ...
                          {searchResults.generalComments.map(c => <CommentMiniCard key={`gc-${c.id}`} comment={c} currentUser={defaultAuthor} onDelete={handleDeleteComment} />)}
                        </div>
                      )}
                    </>
                  )}
          ...
          <Col xs={24} lg={7}>
            ...
            >
              <Descriptions size="small" column={1} style={{ fontSize: 11 }}>
                <Descriptions.Item label={<Text type="secondary" style={{ fontSize: 11 }}>Resource Status</Text>}>
                  <Tag bordered={false} color={selectedResource?.isActive === false ? 'red' : 'green'} style={{ fontSize: 10 }}>
                    {selectedResource?.isActive === false ? 'Inactive' : 'Active'}
                  </Tag>
                </Descriptions.Item>
