...
function ProcessTab({ rows, setRows }: ProcessTabProps) {
  ...
  // Upload — S.No auto-assigned, Start Date parsed from Excel date cells
  const handleUpload = async (file: File) => {
    try {
      ...
      setRows(prev => {
        ...
        const mapped: ProcessRow[] = json.map((r, i) => ({
        ...
        message.success(`${mapped.length} record(s) uploaded`);
        return [...prev, ...mapped];
      });
  ...
  const handleDelete = (key: string) => setRows(prev => prev.filter(r => r.key !== key).map((r, i) => ({ ...r, sno: i + 1 })));
  const handleSave = () => {
    form.validateFields().then(vals => {
      if (editingRow) {
        ...
        setRows(prev => [...prev, {
          key: `pr_${Date.now()}`, sno: prev.length + 1,
    ...
    },
    {
      ...
      render: (_: any, r: ProcessRow) => (
        <Space size={3}>
          ...
          <Popconfirm title="Delete this record?" description="This action cannot be undone." onConfirm={() => handleDelete(r.key)} okText="Delete" cancelText="Cancel" okButtonProps={{ danger: true, size: 'small' }} cancelButtonProps={{ size: 'small' }}>
        ...
        <Tabs
          ...
          items={[
            ...
            },
            {
              ...
              children: <ProcessTab rows={processRows} setRows={setProcessRows} />,
            },
            {
              key: 'insights',
