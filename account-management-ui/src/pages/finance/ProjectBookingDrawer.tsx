...
import { Alert, Button, Drawer, Input, InputNumber, message, Popconfirm, Select, Space, Spin, Table, Tag, Tooltip, Typography, Upload } from 'antd';
import { CalendarOutlined, DeleteOutlined, DownloadOutlined, FileExcelOutlined, FullscreenExitOutlined, FullscreenOutlined, PlusOutlined, UploadOutlined } from '@ant-design/icons';
import * as XLSX from 'xlsx';
import * as financeApi from '../../api/financeApi';
import { downloadBookingTemplate, exportBookingHistory } from './bookingExportUtils';
import { validateSingleProjectBookingUploadRows } from './bookingUploadUtils';
...
interface ProjectBookingDrawerProps {
  open: boolean;
  row: BookingRow | null;
  monthHeaders: string[];
...
export default function ProjectBookingDrawer({
  open,
  row,
  monthHeaders,
  fmtRev,
  ...
  const resetLocalState = () => {
    ...
    setBkBreakdown({});
    setBkHistoryFilter('');
    setBkFilterMilestone(undefined);
    setBkFilterBookedIn(undefined);
    setExpanded(false);
  ...
  useEffect(() => {
    ...
    financeApi.getBookings(row.id).then(b => {
      ...
      setBookingsLoading(false);
      console.error('[ProjectBookingDrawer] Failed to load booking history', error);
  ...
  const availableForMs = (m: string) =>
    Math.max(0, (row.revenue[monthHeaders.indexOf(m)] || 0) - (bookedPerMs[m] || 0));

  const allBookedMonths = monthHeaders.filter(m =>
    (row.revenue[monthHeaders.indexOf(m)] || 0) > 0
  );
  ...
  const handleBookingUpload = async (file: File) => {
    setBkUploadErrors(null);
    try {
      ...
      const { validRows, errors } = validateSingleProjectBookingUploadRows(
        json,
        availableForMs,
        row.milestoneTypes,
      );
      if (errors.length) { setBkUploadErrors(errors); return false; }
      if (!validRows.length) { setBkUploadErrors(['No valid rows found to import.']); return false; }

      setBkSaving(true);
      const result = await financeApi.addBookingsBatch(row.id!, validRows.map(v => ({
        ...v,
        created_by: currentUsername,
      })));
      setBkSaving(false);
      if (result.ok) {
        message.success(`${validRows.length} booking entr${validRows.length === 1 ? 'y' : 'ies'} imported successfully.`);
  ...
  return (
    <Drawer
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          <Space>
            ...
            <Tag style={{ fontSize: '10px', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {row.code || row.project}
            </Tag>
          </Space>
          <Tooltip title={expanded ? 'Collapse' : 'Expand'} overlayInnerStyle={{ fontSize: '11px' }}>
            <Button
              type="text"
              size="small"
              icon={expanded ? <FullscreenExitOutlined style={{ fontSize: '13px' }} /> : <FullscreenOutlined style={{ fontSize: '13px' }} />}
      ...
      placement="right"
      width={expanded ? '65vw' : 440}
      open={open}
      onClose={() => { resetLocalState(); onClose(); }}
      mask={false}
    ...
    >
      <div style={{ display: expanded ? 'grid' : 'flex', gridTemplateColumns: expanded ? '1fr 1fr' : undefined, flexDirection: expanded ? undefined : 'column', gap: 14, height: '100%' }}>
        {canEdit && (
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 8, padding: '10px 12px' }}>
                ...
                <div style={{ fontSize: '10px', color: '#8c8c8c', marginBottom: 2 }}>
                  ...
                  {fullyBookedMonths.length > 0 && (
                    <Tooltip title={`Fully booked: ${fullyBookedMonths.join(', ')}`} overlayInnerStyle={{ fontSize: '11px' }}>
                      <Tag color="red" style={{ fontSize: '10px', marginLeft: 6, cursor: 'default' }}>{fullyBookedMonths.length} fully booked</Tag>
                    </Tooltip>
                  )}
                </div>
                <Select
                  ...
                  options={selectableMonths.map(m => ({
                    value: m,
                    label: `${m} — avail. ${fmtRev(availableForMs(m))} / ${fmtRev(row.revenue[monthHeaders.indexOf(m)] || 0)}`,
                  }))}
                />
              </div>
              <div style={{ gridColumn: '1 / -1' }}>
            ...
            <Button
              ...
              onClick={async () => {
                if (!row.id || bkMilestoneMonths.length === 0 || !bkBookingMonth || !bkAmount) return;
                setBkSaving(true);
        ...
        <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 8, padding: '10px 12px', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <Space size={4}>
              ...
              {canEdit && bookings.length > 0 && (
                <Popconfirm
                  ...
                  onConfirm={async () => {
                    if (!row.id) return;
                    await financeApi.deleteAllProjectBookings(row.id, currentUsername);
                    financeApi.getBookings(row.id).then(setBookings);
                  }}
                  okText="Delete All"
                ...
                >
                  <Button type="link" size="small" danger style={{ fontSize: '10px', padding: 0 }}>Delete All</Button>
                </Popconfirm>
              )}
              {canEdit && (
                <Tooltip title="Upload bookings (.xlsx)" overlayInnerStyle={{ fontSize: '11px' }}>
                  <Upload accept=".xlsx,.xls" showUploadList={false} beforeUpload={f => handleBookingUpload(f as File)}>
                  ...
                  onClick={async () => {
                    ...
                    downloadBookingTemplate(
                      allBookedMonths.map(m => ({
                        milestoneMonth: m,
                        totalAmount: row.revenue[monthHeaders.indexOf(m)] || 0,
                        ...
                        milestoneType: milestoneTypes[m] || 'booked',
                      })),
                      row.project,
                      row.code,
                    );
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '4px 6px', marginBottom: 8, flexShrink: 0 }}>
            <Select size="small" allowClear placeholder="Milestone…" value={bkFilterMilestone} onChange={v => setBkFilterMilestone(v)} style={{ fontSize: '11px' }} options={histMilestoneOpts.map(m => ({ value: m, label: m }))} popupMatchSelectWidth={false} />
            <Select size="small" allowClear placeholder="Booked In…" value={bkFilterBookedIn} onChange={v => setBkFilterBookedIn(v)} style={{ fontSize: '11px' }} options={histBookedInOpts.map(m => ({ value: m, label: m }))} popupMatchSelectWidth={false} />
            <Input size="small" allowClear placeholder="Search…" value={bkHistoryFilter} onChange={e => setBkHistoryFilter(e.target.value)} style={{ fontSize: '11px' }} prefix={<span style={{ color: '#bbb', fontSize: '10px' }}>⌕</span>} />
          </div>
          {bookingsLoading ? (
              ...
              columns={[
                ...
                ...(canEdit ? [{
                  ...
                  render: (_: any, rec: financeApi.ProjectBooking) => (
                    <Popconfirm title="Delete this booking?" onConfirm={async () => { await financeApi.deleteBooking(row.id!, rec.id, currentUsername); financeApi.getBookings(row.id!).then(setBookings); }} okText="Delete" okButtonProps={{ danger: true, size: 'small' }} cancelButtonProps={{ size: 'small' }}>
