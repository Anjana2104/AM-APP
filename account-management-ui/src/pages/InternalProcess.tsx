...
import PiwUploadSubTabPanel from './internal-process/PiwUploadSubTabPanel';
import PiwCreateTabPanel from './internal-process/PiwCreateTabPanel';
import { mapProcessApiRow, resequenceRows, toProcessBulkSavePayload, toProcessExportRows } from './internal-process/processRowMappers';
import {
...
function recencySortKey(row: Pick<ProcessRow, 'updatedAt' | 'createdAt'>): number {
...
function ProcessTab({ rows, setRows, fromServer, setFromServer, resourceRefreshKey = 0, initialSow, initialFilters, resetFiltersSignal = 0 }: ProcessTabProps) {
  ...
  const mapProcRes = (r: any): ProcRes => {
    const normalized = mapResourceApiRowToPayload(r);
    return {
      id: Number(normalized.id ?? r.id ?? 0),
  ...
  const handleDelete = (r: ProcessRow) => {
    setRows(prev => resequenceRows(prev.filter(pr => pr.key !== r.key)));
  ...
  const handleToggleActive = async (row: ProcessRow) => {
    ...
    setRows(prev => prev.map(r => r.key === row.key ? { ...r, active: newVal, updatedAt: nowIso } : r));
    if (detailRow && detailRow.key === row.key) setDetailRow(prev => prev ? { ...prev, active: newVal, updatedAt: nowIso } : prev);
  ...
  const handleSave = () => {
    form.validateFields().then(async vals => {
      ...
      if (editingRow) {
        const updatedRow = { ...editingRow, ...rowVals, updatedAt: new Date().toISOString() };
        ...
        if (editingRow.id) {
          try {
            const normalizedNext = {
              sow: rowVals.sow || '',
              startDate: rowVals.startDate || '',
              signedSow: rowVals.signedSow || '',
              piw: rowVals.piw || '',
              active: rowVals.active || '',
              salesforceId: rowVals.salesforceId || '',
              promsId: rowVals.promsId || '',
              budget: rowVals.budget || '',
              openAirCode: rowVals.openAirCode || '',
              eprev: rowVals.eprev || '',
              comments: rowVals.comments || '',
              accountAnchor: rowVals.accountAnchor || '',
            } as const;
            const changedPayload: Partial<processApi.ProcessPayload> = {};
            (Object.keys(normalizedNext) as Array<keyof typeof normalizedNext>).forEach((k) => {
              if (String(editingRow[k] || '') !== String(normalizedNext[k] || '')) {
                (changedPayload as Record<string, string>)[k] = normalizedNext[k];
              }
            });
            ...
            if (fs && latestRows.length > 0) {
              ...
              const latestRecord = latestMapped.find(r => r.id === editingRow.id);
              if (latestRecord) {
                setRows(prev => prev.map(r => (
                  r.id === editingRow.id
                    ? { ...r, ...latestRecord, key: r.key, sno: r.sno }
                    : r
                )));
  ...
  const displayed = useMemo(() => [...rows]
    ...
    if (filters.accountAnchor === '__UNASSIGNED__' && (r.accountAnchor || '').trim()) return false;
    if (filters.accountAnchor && filters.accountAnchor !== '__UNASSIGNED__' && (r.accountAnchor || '') !== filters.accountAnchor) return false;
    if (filters.accountAnchorPresent === 'Yes' && !(r.accountAnchor || '').trim()) return false;
    if (filters.startDateFrom && dateSortKey(r.startDate || '') < dateSortKey(filters.startDateFrom || '')) return false;
    if (filters.startDateTo && dateSortKey(r.startDate || '') > dateSortKey(filters.startDateTo || '')) return false;
  ...
  // Pipeline view: already recency-sorted via displayed
  const pipelineRows = useMemo(
    () => displayed,
    [displayed]
  );

  // Owner options: from top-most config linked to ra_process_account_anchor_field
  const anchorOptions = useMemo(() => {
    const linkedConfigs = configs.filter(c => (c.linkedTo ?? []).includes('ra_process_account_anchor_field'));
  ...
  const handleAllocateSave = () => {
    ...
    if (allocateSingleRow) {
      ...
      const nowIso = new Date().toISOString();
      setRows(prev => prev.map(r => r.key === allocateSingleRow.key ? { ...r, accountAnchor: allocateAnchor, updatedAt: nowIso } : r));
      ...
      const nowIso = new Date().toISOString();
      setRows(prev => prev.map(r => allocateSelected.includes(r.key) ? { ...r, accountAnchor: allocateAnchor, updatedAt: nowIso } : r));
  ...
  const tableCols = [
    ...
    visibleColumns.startDate  && { title: 'Start Date', dataIndex: 'startDate', key: 'startDate', width: 90, sorter: (a: ProcessRow, b: ProcessRow) => dateSortKey(a.startDate || '') - dateSortKey(b.startDate || ''), onHeaderCell: () => ({ style: hStyle }), render: (v: string) => <span style={cStyle}>{v}</span> },
    ...
    visibleColumns.accountAnchor && { title: 'Owner', dataIndex: 'accountAnchor', key: 'accountAnchor', width: 130, sorter: (a: ProcessRow, b: ProcessRow) => (a.accountAnchor || '').localeCompare(b.accountAnchor || ''), onHeaderCell: () => ({ style: hStyle }),
      render: (v: string) => v
        ...
        <Dropdown
          menu={{
            items: [
              ...
              { key: 'anchor', label: <span style={{ fontSize: '11px' }}>Assign Owner</span>, icon: <IdcardOutlined style={{ fontSize: '11px' }} />, onClick: () => { setAllocateSingleRow(r); setAllocateAnchor(r.accountAnchor || ''); setAllocateModal(true); } },
              { type: 'divider' as const },
              canDelete ? {
                key: 'delete', label: <span style={{ fontSize: '11px' }}>Delete</span>,
                icon: <DeleteOutlined style={{ fontSize: '11px' }} />, danger: true,
    ...
    <div ref={filterPanelRef} style={{ width: 240, flexShrink: 0, background: '#fafafa', borderRadius: 8, padding: 16, border: '1px solid #f0f0f0', alignSelf: 'flex-start' }}>
      ...
      <Space direction="vertical" style={{ width: '100%' }} size={8}>
        ...
        <div>
          ...
          <DatePicker
            size="small"
            value={filters.startDateFrom ? dayjs(filters.startDateFrom) : null}
            onChange={(v) => setFilters(f => ({ ...f, startDateFrom: v ? v.format('YYYY-MM-DD') : '' }))}
            allowClear
            style={{ width: '100%', fontSize: '11px' }}
          />
        </div>
        <div>
          ...
          <DatePicker
            size="small"
            value={filters.startDateTo ? dayjs(filters.startDateTo) : null}
            onChange={(v) => setFilters(f => ({ ...f, startDateTo: v ? v.format('YYYY-MM-DD') : '' }))}
            allowClear
            style={{ width: '100%', fontSize: '11px' }}
          />
        </div>
        ...
        <Form form={form} layout="vertical" size="small">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 12px' }}>
            ...
            <Form.Item name="accountAnchor" label={<span style={{ fontSize: '11px' }}>Owner</span>}>
              <Select
                showSearch allowClear placeholder="Select owner"
                style={{ fontSize: '12px' }}
                options={anchorOptions}
                notFoundContent={<span style={{ fontSize: '11px', color: '#8c8c8c' }}>No owners configured</span>}
              />
            </Form.Item>
          </div>
              ...
              <Select
                ...
                notFoundContent={
                  <div style={{ padding: '8px 12px', fontSize: '11px', color: '#8c8c8c' }}>
                    No owners configured — add them in Configuration
                  </div>
                }
              />
            </div>
...
export function InternalProcess({ resources = [], initialSow }: { resources?: ResourceRow[]; initialSow?: string }) {
  ...
  const handleSowDelete = (processKey: string) => {
    setProcessRows(prev => resequenceRows(prev.filter(r => r.key !== processKey)));
  };
                  ...
                  items={[
                    {
                      ...
                      children: <ProcessTab rows={processRows} setRows={setProcessRows} fromServer={fromServer} setFromServer={setFromServer} resourceRefreshKey={resourceRefreshKey} initialSow={initialSow} initialFilters={insightsFilters} resetFiltersSignal={processFilterResetSignal} />,
                    },
                    {
                      key: 'detailview',
